...
namespace Hubo.Tests
{
    public abstract class HuboTestBase : AbpIntegratedTestBase
    {
        ...
        protected void UsingDbContext(Action<HuboDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<HuboDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<HuboDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<HuboDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<HuboDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<HuboDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<HuboDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<HuboDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<HuboDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<HuboDbContext, Task<T>> func)
        {
            T result;
